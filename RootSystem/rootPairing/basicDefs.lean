...
def IsReflexive.toPerfectPairingDual_flip [Module.IsReflexive R M] : PerfectPairing R M (Module.Dual R M) := by
  exact PerfectPairing.flip (IsReflexive.toPerfectPairingDual)

lemma IsReflexive.toPerfectPairingDual_flip_apply [Module.IsReflexive R M] {f : Module.Dual R M} {x : M} :
    IsReflexive.toPerfectPairingDual_flip (R := R) x f = f x := by
  rfl
